...
from libumccr.aws import s3_client, resource
...
def head_s3_object(bucket: str, key: str) -> (bool, dict):
    ...
    """
    try:
        return True, s3_client().head_object(Bucket=bucket, Key=key)
    except ClientError as e:
...
def restore_s3_object(bucket: str, key: str, **kwargs) -> (bool, dict):
    ...
    try:
        resp = s3_client().restore_object(Bucket=bucket, Key=key, RestoreRequest=restore_request)
...
def get_s3_object(bucket: str, key: str, **kwargs) -> (bool, dict):
    ...
    """
    try:
        return True, s3_client().get_object(Bucket=bucket, Key=key, **kwargs)
    except ClientError as e:
...
def get_s3_object_to_bytes(bucket: str, key: str) -> bytes:
    ...
    """
    try:
        obj_body = resource('s3').Object(bucket, key).get()['Body']
        if ".gz" in key:
...
def put_s3_object_tagging(bucket: str, key: str, tagging: dict):
    ...
    :return:
    """
    return s3_client().put_object_tagging(Bucket=bucket, Key=key, Tagging=tagging)
